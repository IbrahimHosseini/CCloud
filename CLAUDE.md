...
- **Focus structure (the cause of most TV bugs here):**
  ...
  - When a screen opens, or the focused element disappears, Android gives focus to the first element at the top left. On TV that is usually the sidebar's Movies item, or Back in the player. When you remove a focused element, move focus somewhere sensible yourself.
...
- **Focus visuals:** the default ripple focus highlight is too faint on a TV. Use `Modifier.focusRing` from `components/FocusRing.kt`, placed before the element's clickable; use `focusOutline` when the focused element is a child, e.g. to outline a whole card while its header has focus.
